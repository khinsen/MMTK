...
class PeptideTest(unittest.TestCase):
    ...
    def test_subspaceModes(self):
        ...
        freq = vmodes.frequencies[6:]
        self.assertAlmostEqual(freq[0],  3.29430105, 4)
        self.assertAlmostEqual(freq[1],  3.5216842, 4)
        self.assertAlmostEqual(freq[2],  6.36658335, 4)
        self.assertAlmostEqual(freq[3],  6.81335036, 4)
        self.assertAlmostEqual(freq[4],  8.96368475, 4)
        self.assertAlmostEqual(freq[5],  9.80151378, 4)
        self.assertAlmostEqual(freq[6],  12.26528168, 4)
        self.assertAlmostEqual(freq[7],  12.64098176, 4)
        self.assertAlmostEqual(freq[8],  16.13558466, 4)
        self.assertAlmostEqual(freq[9],  17.15343073, 4)
        self.assertAlmostEqual(freq[10], 24.86662786, 4)
        self.assertAlmostEqual(freq[11], 29.0265789, 4)
        for i in range(len(vmodes)):
            mi = vmodes.rawMode(i)
            norm_sq = mi.dotProduct(mi)
            self.assertAlmostEqual(norm_sq, 1.)
    ...
    def test_subspaceModesWithExclusion(self):
        ...
        vmodes = NormalModes.VibrationalModes(self.universe,
                                              subspace=(excluded,
                                                        self.subspace))
        freq = vmodes.frequencies[6:]
        self.assertAlmostEqual(freq[0], 3.48436894, 4)
        self.assertAlmostEqual(freq[1], 5.36671021, 4)
        self.assertAlmostEqual(freq[2], 6.57184292, 4)
        self.assertAlmostEqual(freq[3], 8.37798098, 4)
        self.assertAlmostEqual(freq[4], 8.95620773, 4)
        self.assertAlmostEqual(freq[5], 12.06606868, 4)
        self.assertAlmostEqual(freq[6], 12.62157723, 4)
        self.assertAlmostEqual(freq[7], 16.02985338, 4)
        self.assertAlmostEqual(freq[8], 16.90566338, 4)
        self.assertAlmostEqual(freq[9], 23.55890937, 4)
        self.assertAlmostEqual(freq[10],28.92170134, 4)
        self.assertAlmostEqual(freq[11],41.55110572, 4)
        for i in range(len(vmodes)):
            mi = vmodes.rawMode(i)
            norm_sq = mi.dotProduct(mi)
            self.assertAlmostEqual(norm_sq, 1.)
